...
// Google inside the iOS shell must ALSO be native: Google Identity Services
// (the web button) refuses the capacitor://localhost origin, which is why the
// web button silently never rendered in the app. The native flow needs an iOS
// OAuth client id; setting the web client as iOSServerClientId makes the
// returned idToken's audience the WEB client — so /auth/google verifies as-is.
...
async function ensureSocialLogin() {
  ...
  await SocialLogin.initialize({
    google: {
      ...
      iOSServerClientId: GOOGLE_CLIENT_ID || undefined,
    },
  })
  _socialInited = true
}
...
export function AuthModal({ onClose, onSuccess, initialTab = 'signin' }) {
  ...
  async function handleGoogleNative() {
    ...
    try {
      ...
      const out = await googleAuth(idToken)
      onSuccess?.({ token: out.access_token, user: out.user })
    } catch (e) {
      const msg = String(e?.message || '')
      // Surface the underlying reason — "not implemented" means the native
      // plugin wasn't synced; "Invalid Google credential" means the backend
      // rejected the token's audience. Generic text hides the actual problem.
      if (!/cancel/i.test(msg)) setError(`Google sign-in failed${msg ? ` (${msg.slice(0, 90)})` : ''}. Try email + password.`)
    } finally { setBusy(false) }
  }
      ...
      <div onClick={e => e.stopPropagation()} style={sheetInner}>
        ...
        {tab !== 'forgot' && (
          <div style={{ padding: '0 20px 14px' }}>
            ...
            {IS_IOS_NATIVE ? (
              // Native Google button — the GIS web button can't mount on the
              // capacitor:// origin. Hidden entirely if the iOS client id isn't set.
              GOOGLE_IOS_CLIENT_ID ? (
                <button
                  ...
                  style={{
                    ...
                    border: '1px solid var(--gray-200)', borderRadius: 12, cursor: 'pointer',
                    fontSize: 15, fontWeight: 600, fontFamily: 'inherit',
