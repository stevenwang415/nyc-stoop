...
| 17 | Saved-plan view: travel connectors, day summaries, notes | ✅ done 07-15 | The saved plan is the page people read standing on a sidewalk, so it now answers the sidewalk questions: per-leg travel hints between cards (🚶 ~7 min walk / 🚇 with the live page's subway detail — same `estimateTravelCoords` + `findSubwayLeg`, so the two views can never disagree), a per-day summary line under each day pill (🚶/🚇/🚕 totals + ≈$/person from `MEAL_PRICE_RANGE`), and the user's own card notes shown read-only in italics (✎, stops keyed by id, meals by `rest:<id>` — the "order the tsukune" note appears at the table). Deliberately NOT added: editing, suggestions, anything that unsettles a settled plan. |
